...
abstract class AbstractController extends SessionManager implements ControllerInterface
{
     ...
     */
    public function getView($directory = '', $view = '', $layout = true)
    {
    ...
    /**
     * Search $confiFile in config directory and if $key is not null, return the value of $key in $configFile
     * Return an empty array if $configFile is empty or if it was not found
     * @param string $configFile
     * @param string|int $key
     * @return array
     */
    protected function getConfig($configFile, $key = null)
    {
        $configFileContent = [];

        if (file_exists(__DIR__ . '/../../../config/' . $configFile . '.conf.php')) {
            $configFileContent = require (__DIR__ . '/../../../config/' . $configFile . '.conf.php');

            if ((is_string($key) || is_int($key)) && array_key_exists($key, $configFileContent)) {
                $configFileContent = $configFileContent[$key];
            }
        }
    ...
    /**
     * Redirect to /Controller_Name/Controller_Action_Name.
     *
